<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=220&section=header&text=Sheynner%20Zavala&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Analista%20de%20Datos%20%26%20Desarrollador%20Full-Stack&descAlignY=55&descSize=20" width="100%"/>

<a href="https://sheynner-dev.github.io/cv/">
  <img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=22&duration=2500&pause=800&color=F70000&background=FF162800&width=900&lines=Transformo+datos+complejos+en+soluciones+de+alto+impacto;Python+|+Machine+Learning+|+Power+BI;React+|+Node.js+|+Microsoft+Azure;Del+analisis+estadistico+al+software+de+produccion" alt="Typing SVG" />
</a>

<br/>

[![Portafolio](https://img.shields.io/badge/Portafolio-FF6B00?style=for-the-badge&logo=googlechrome&logoColor=white&labelColor=1a1a1a)](https://sheynner-dev.github.io/cv/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=1a1a1a)](https://www.linkedin.com/in/sheynner-zavala-48588832a/)
[![Gmail](https://img.shields.io/badge/zavalasheynner@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white&labelColor=1a1a1a)](mailto:zavalasheynner@gmail.com)

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0F2027,100:2C5364&height=3&width=100%"/>

</div>

---

## Engineering Profile

Analista de datos y desarrollador Full Stack enfocado en crear soluciones escalables mediante análisis de datos, inteligencia artificial y computación en la nube.

Especializado en transformar datos sin procesar en sistemas de apoyo a la toma de decisiones utilizando aprendizaje automático (Machine Learning), inteligencia empresarial (Business Intelligence) y arquitecturas de software modernas.

Motivado por la automatización, la optimización del rendimiento y la creación de productos digitales de alto impacto.

- Estudiante de Ingeniería de Software en **SENATI - Talara** (2023 - actualidad)
- Enfocado en **Data & AI**: Python, Machine Learning, Power BI y analítica aplicada a la toma de decisiones
- Desarrollo **Full-Stack** con React, Node.js, Vite, Tailwind CSS y despliegue en **Microsoft Azure**
- Base sólida en **ciberseguridad** e infraestructura (Cisco, Red Hat, AWS)

---

# AI • Data • Cloud • Software

<div align="center">

<table>
<tr>
<td align="center" width="50%">

### Data Analytics

<p>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="52"/>
&nbsp;
<img src="https://upload.wikimedia.org/wikipedia/commons/c/cf/New_Power_BI_Logo.svg" width="52"/>
&nbsp;
<img src="https://img.icons8.com/color/96/microsoft-excel-2019.png" width="52"/>
</p>

</td>

<td align="center" width="50%">

### Machine Learning

<p>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/opencv/opencv-original.svg" width="52"/>
&nbsp;
<img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" width="52"/>
&nbsp;
<img src="https://cdn.simpleicons.org/keras/D00000" width="52"/>
</p>

</td>
</tr>

<tr>
<td align="center">

### Full Stack

<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/androidstudio/androidstudio-original.svg" width="52"/>

</p>

</td>

<td align="center">

### Cloud & Databases

<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqlite/sqlite-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="52"/>

</p>

</td>
</tr>

<tr>
<td colspan="2" align="center">

### Cybersecurity & Tools

<p>

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.simpleicons.org/kalilinux/557C94" width="52"/>
&nbsp;
<img src="https://cdn.simpleicons.org/wireshark/1679A7" width="52"/>
&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="52"/>
&nbsp;
<img src="https://cdn.simpleicons.org/figma/F24E1E" width="52"/>

</p>

</td>
</tr>
</table>

</div>


## Proyectos Destacados

### Clasificador de Residuos con IA
Sistema de clasificación inteligente de residuos mediante Deep Learning. Procesamiento de 50,000 imágenes para entrenar un modelo CNN con TensorFlow/Keras, alcanzando 94% de precisión en la identificación de 5 categorías de residuos, con inferencia en tiempo real por debajo de los 100ms.

`Python` `TensorFlow` `Keras` `OpenCV` `Machine Learning`

### Detector de Acné Médico
Herramienta de análisis dermatológico basada en procesamiento de imágenes médicas. Aplica visión por computadora para detectar y clasificar tipos de acné, con generación automática de reportes en PDF y gestión de historial de pacientes.

`Python` `PyQt` `OpenCV` `Image Processing`

### App Móvil CIE-10 MINSA
Aplicación móvil para consulta del catálogo CIE-10 usada por profesionales de la salud. Transformación de datos CSV a una base SQLite optimizada, con motor de búsqueda por debajo de 500ms y funcionamiento completamente offline.

`Android` `Java` `SQLite` `Data Processing`

### Sistema Web - Área de Terapia (MINSA Talara II)
Sistema web para la gestión de pacientes del área de terapia, con almacenamiento y análisis de datos, reportes estadísticos y control de acceso por roles de usuario.

`PHP` `MySQL` `HTML/CSS` `Reporting`

### Sistema CRUD de Estudiantes
Sistema web con operaciones CRUD completas para la gestión de registros de estudiantes, validación de datos y almacenamiento estructurado en MySQL.

`PHP` `MySQL` `Data Validation`

### App Móvil para Cálculo de IMC
Aplicación Android para el cálculo y seguimiento del Índice de Masa Corporal, con historial de datos, análisis de tendencias y recomendaciones basadas en datos.

`Android` `Kotlin` `SQLite` `Analytics`

---

## Experiencia Profesional

| Rol | Empresa | Periodo |
|---|---|---|
| Asistente de Negocios | Mibanco | Ene 2026 - Abr 2026 |
| Practicante en Estadística e Informática / Unidad de Seguros | Centro de Salud Talara II | Ago 2025 - Nov 2025 |
| Practicante en Área de Estadística e Informática | Centro de Salud Talara II | Mar 2025 - Jun 2025 |

---

## Educación

**Ingeniería de Software** — SENATI, Talara (2023 - actualidad)
Formación en desarrollo de sistemas empresariales, arquitectura de software, metodologías ágiles, modelado de bases de datos relacionales y principios de ciberseguridad.

**Educación Secundaria** — I.E. José Pardo y Barreda, Talara (2018 - 2022)

---

## Certificaciones

![AWS](https://img.shields.io/badge/AWS_Academy-Cloud_Foundations-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-Introduction_to_Cybersecurity-1BA0D7?style=flat-square&logo=cisco&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-Python_Essentials_1-1BA0D7?style=flat-square&logo=cisco&logoColor=white)
![Red Hat](https://img.shields.io/badge/Red_Hat-System_Administration_I-EE0000?style=flat-square&logo=redhat&logoColor=white)
![Docker](https://img.shields.io/badge/Cognitive_Class-Docker_Essentials-2496ED?style=flat-square&logo=docker&logoColor=white)
![HackerRank](https://img.shields.io/badge/HackerRank-Problem_Solving-2EC866?style=flat-square&logo=hackerrank&logoColor=white)

*Certificaciones adicionales en Big Data, SQL, C++, IoT y Automatización Industrial (PLC / Node-RED) disponibles en el portafolio.*

---

## Idiomas

- **Español** — Nativo
- **Inglés** — Básico

<br/>

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:2C5364,50:203A43,100:0F2027&height=120&section=footer" width="100%"/>
</div>
